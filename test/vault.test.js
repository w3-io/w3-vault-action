...
import { ENVIRONMENTS, CCTP_DOMAINS, USDC_ADDRESSES, METHODS } from '../src/contracts.js'

describe('resolveEnvironment', () => {
  it('resolves testing', () => {
    const env = resolveEnvironment('testing')
    assert.equal(env.name, 'testing')
    assert.ok(env.operator.startsWith('0x'))
...
describe('contract addresses', () => {
  ...
  it('CCTP domains cover major chains', () => {
    assert.equal(CCTP_DOMAINS.ethereum, 0)
    assert.equal(CCTP_DOMAINS.base, 6)
    assert.ok(CCTP_DOMAINS.arbitrum !== undefined)
    assert.ok(CCTP_DOMAINS.polygon !== undefined)
  })

  it('USDC addresses exist for all CCTP chains', () => {
    for (const chain of Object.keys(CCTP_DOMAINS)) {
      if (chain === 'noble' || chain === 'solana') continue // not EVM
      assert.ok(USDC_ADDRESSES[chain], `Missing USDC address for ${chain}`)
    }
  })

  it('method signatures are valid function signatures', () => {
    for (const [name, sig] of Object.entries(METHODS)) {
      assert.ok(sig.startsWith('function '), `${name} should start with "function "`)
    }
  })
})
